...
dataset_all = ['CyberShake_30', 'Montage_25', 'Inspiral_30', 'Sipht_30', \
                'CyberShake_50', 'Montage_50', 'Inspiral_50', 'Sipht_60',\
                'CyberShake_100', 'Montage_100', 'Inspiral_100', 'Sipht_100']

dataset_dict = {'S': dataset_30, 'M': dataset_50, 'L': dataset_100, 'XL': dataset_1000, 'all': dataset_all}
...
class dataset:
    def __init__(self, arg,vm_types):
        ...
        base_apps = ['CyberShake', 'Montage', 'Inspiral', 'Sipht']
        repeat = 3 if arg == 'all' else 1
        for app_name, dataset_name in zip(base_apps * repeat, dataset_dict[arg]):
            dag, wsetProcessTime = processDAG.buildGraph(
                app_name,
                parentdir + f'/workflow_scheduling_v3/dax/{dataset_name}.xml',
            )
