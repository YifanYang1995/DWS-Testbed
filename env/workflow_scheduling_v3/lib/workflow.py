...
class Workflow:
    def __init__(self, generateTime1, app, appID, originDC, time, totalT, index): #, ratio):
        ...
        self.appID = appID  # Application/workflow type identifier.
    ...
    def get_allsucc2end(self, start):       # Sum task sizes over every descendant of start.
        G = self.app
        exit = [n for n, d in G.out_degree() if d == 0]     # Exit nodes.
        nodes_in_paths = set()
        for leaf in exit:
            for path in nx.all_simple_paths(G, source=start, target=leaf):
