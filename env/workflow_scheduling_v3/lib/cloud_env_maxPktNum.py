...
class cloud_simulator(object):
    ...
    def _init(self):
        ...
        self.all_workload = np.zeros((self.wfNum))
    ...
    def workflow_generator(self, usr, wfID, nextarrT):
        ...
        self.all_workload[self.wfIndex] = self.set.dataset.wsetTotProcessTime[wfID]
    ...
    def get_nextTimeStep(self):
        ...
        if tempnextTimeStep > tempnextTimeStep1:  # task ready time > VM minimum time
            ...
            if tempnextTimeStep == math.inf:  # No queued workflow arrival remains.
                self.nextTimeStep = None  # No VM completion event remains.
                self.nextUsr = None
                self.nextWrf = None
                self.nextisUsr = True
            else:
    ...
    def state_info_construct1(self):  #
        # Replace utilization for empty VM slots with the current mean utilization.
        ...
        for i,vm in enumerate(self.vm_queues):
            ...
            if len(temp_vm_edges) > 0:
                ...
                vm_lastIdx.append([])
            # Prepare the values appended to wf_features.
