...
class Memory: # state, action, reward, done, prob, val
    ...
    def compute_returns(self):  # standard
        ...
        discounted_sums = np.zeros_like(norm_reward_mb)
        ...
        return discounted_sums[:len(norm_reward_mb) - configs.batch_size*4]

    def compute_returns_new(self):  # with slip window & gae=1
        norm_reward_mb = np.array(self.reward_mb) / configs.normalize_rewards
        ...
        if configs.slip_return==1:
            discounted_sums = discounted_sums[:len(norm_reward_mb) - configs.batch_size]
        # Assign the returns
...
class Critic(nn.Module):
    ...
    def prepare_input(self, A, mask_A, batch_index, candidate):
        # Reorganize A and mask_A into (batch_size, sequence_length, embedding_dim).
        ...
        # Locate each candidate in original_idx.
        candidate_positions = []
        for c in candidate:
    ...
    def forward(self,
        ...
        if deterministic is True:
            ...
            for i in range(arr.size(0)):
                ...
                random_choice = torch.randint(len(max_positions), (1,), device=arr.device).item()
        ...
        for _ in range(configs.n_epochs):
            ...
            for rollout_data in actor_samples.get(configs.batch_size):
                ...
                vals = self.critic(state_wf = batch_states.wf_features,
                                ...
                                candidate_task_index = batch_states.candidate_taskID,
                                deterministic = False)
                v_loss = F.mse_loss(rollout_data.returns, vals)  # PPO critic loss.
